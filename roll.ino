...
void sidewind(String Direction) {
  int delayTime = 100;
  int Multiplier = (Direction == "Left") ? 1 : -1;
  ...
  for (int i = 0; i < 360; i++) {
    ...
    for (int j = 0; j < 7; j++) {  
      // X 轴舵机（奇数）负责侧向运动
      int posX = 500 + offset + amplitude * sin(Speed * rads + j * Wavelengths * shift - (Multiplier - 1) * pi / 4);
      LobotSerialServoMove(mySerial, 2*j + 1, posX, delayTime);

      // Y 轴舵机（偶数）保持静止
      //int posY = 500;  // 保持中心位置，或者可以添加轻微运动
      // Y 轴舵机做轻微运动，下面的代码
       int posY = 500 + small_amplitude * sin(Speed * rads + j * Wavelengths * shift);
      LobotSerialServoMove(mySerial, 2*j + 2, posY, delayTime);
    }
    delay(10);
  }
}
